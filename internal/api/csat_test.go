...
func TestListCSATResponses(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := client.ListCSATResponses(context.Background(), tt.params)
			...
			if !tt.expectError && len(result.Payload) != tt.expectCount {
				t.Errorf("Expected %d responses, got %d", tt.expectCount, len(result.Payload))
			}
		})
	}
}
...
func TestGetConversationCSAT(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "conversation without CSAT",
			conversationID: 456,
			statusCode:     http.StatusOK,
