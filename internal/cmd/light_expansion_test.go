...
func TestInboxesListCommand_Light(t *testing.T) {
	handler := newRouteHandler().
		On("GET", "/api/v1/accounts/1/inboxes", jsonResponse(200, `{
			"payload": [
				{"id": 48, "name": "Amy Shop LINE", "channel_type": "Channel::Api", "avatar_url": "https://long-url", "greeting_enabled": false, "enable_auto_assignment": true}
			]
		}`))
	...
	if item.ID != 48 || item.Name != "Amy Shop LINE" || item.ChannelType != "Channel::Api" {
